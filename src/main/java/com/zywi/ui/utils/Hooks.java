...
public class Hooks extends BaseClass {
	...
	/**
	 * Executes after each scenario: handles screenshot for failures, closes
	 * browser, and logs status.
	 */
	@After
	...
	public static void afterAll() {
	    // Check and compute execution duration
	    if (suiteStartTime != null) {
	        LocalDateTime suiteEndTime = LocalDateTime.now();

	        //  Compute the test suite duration
	        ...
	        try (PrintWriter writer = new PrintWriter(new FileWriter("target/logs/automation.log", true))) {
	        	...
	        	logger.warn("Unable to write test summary to automation.log file: " + e.getMessage());
	            e.printStackTrace();
	        }
